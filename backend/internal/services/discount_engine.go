...
func RunDiscountEngine() ([]models.DiscountSuggestion, error) {
	...
	var suggestions []models.DiscountSuggestion
...
func evaluateProduct(p models.Product) *models.DiscountSuggestion {
	...
	if p.Estoque > 50 && p.DemandaMediaDiaria < 0.5 {
		return &models.DiscountSuggestion{
			ProductID:         p.ID,
			...
			Status:            models.DiscountStatusPending,
		}
	}
...
func ApproveSuggestion(suggestionID uint, userID string) error {
	var suggestion models.DiscountSuggestion
	if err := database.DB.First(&suggestion, suggestionID).Error; err != nil {
		return err
	}

	suggestion.Status = models.DiscountStatusApproved
...
func RejectSuggestion(suggestionID uint, userID string) error {
	...
	suggestion.Status = models.DiscountStatusRejected
	suggestion.ReviewedBy = userID

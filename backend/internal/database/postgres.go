...
func ConnectDB() (*pgxpool.Pool, error) {
	...
	// String de conexão DSN do PostgreSQL
	...
	if err != nil {
		return nil, fmt.Errorf("erro ao criar pool de conexões pgxpool: %w", err)
	}

	// Verifica a conectividade pingando o banco
	if err := pool.Ping(ctx); err != nil {
		pool.Close()
		return nil, fmt.Errorf("falha ao pingar banco de dados PostgreSQL: %w", err)
	}
